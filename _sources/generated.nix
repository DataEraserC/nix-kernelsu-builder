# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  anykernel-kernelsu = {
    pname = "anykernel-kernelsu";
    version = "ee30bad85a483cad2b1e4ff34e69f983a04f875a";
    src = fetchFromGitHub {
      owner = "Kernel-SU";
      repo = "AnyKernel3";
      rev = "ee30bad85a483cad2b1e4ff34e69f983a04f875a";
      fetchSubmodules = false;
      sha256 = "sha256-QJS4+1teNcgUOy2gD+KuoEG/9pUNd/6uSH84AkAlVoQ=";
    };
    date = "2023-10-30";
  };
  anykernel-osm0sis = {
    pname = "anykernel-osm0sis";
    version = "fdf4603a407048740191a478229b294a714949f8";
    src = fetchFromGitHub {
      owner = "osm0sis";
      repo = "AnyKernel3";
      rev = "fdf4603a407048740191a478229b294a714949f8";
      fetchSubmodules = false;
      sha256 = "sha256-8fBGZj5xXI08k38aivVebpLyWM4q3ghOr+ZhxmVm+R0=";
    };
    date = "2024-10-14";
  };
  gcc-aarch64-linux-android = {
    pname = "gcc-aarch64-linux-android";
    version = "5797d7f622321e734558bd3372a9ab5ad6e6a48e";
    src = fetchFromGitHub {
      owner = "kindle4jerry";
      repo = "aarch64-linux-android-4.9-bakup";
      rev = "5797d7f622321e734558bd3372a9ab5ad6e6a48e";
      fetchSubmodules = false;
      sha256 = "sha256-ZrQmFyiDOKg+qcgdpZqtz+LgDDaao2W27kdZZ2As8XU=";
    };
    date = "2020-02-22";
  };
  gcc-arm-linux-androideabi = {
    pname = "gcc-arm-linux-androideabi";
    version = "3ecb542702c2ca0e502533c3f6d02f0f06f584f1";
    src = fetchFromGitHub {
      owner = "KudProject";
      repo = "arm-linux-androideabi-4.9";
      rev = "3ecb542702c2ca0e502533c3f6d02f0f06f584f1";
      fetchSubmodules = false;
      sha256 = "sha256-5aF2Pl+h6J8/5TfQf2ojp3FCnoKakWH6KBCkWdy5ho8=";
    };
    date = "2019-11-25";
  };
  kernelsu-gki-stable = {
    pname = "kernelsu-gki-stable";
    version = "c9edb7d3be11e75008f247a4458bf16e6ef52557";
    src = fetchgit {
      url = "https://github.com/tiann/KernelSU.git";
      rev = "c9edb7d3be11e75008f247a4458bf16e6ef52557";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-tYA5937Y7VENesTcFAiKKMbz4HaVH61XGUlZ4PhIbzY=";
    };
    date = "2024-11-22";
  };
  kernelsu-stable = {
    pname = "kernelsu-stable";
    version = "v0.9.5";
    src = fetchgit {
      url = "https://github.com/tiann/KernelSU.git";
      rev = "v0.9.5";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-pvaL6KEB7X3s8zyLQSPhBhoXaNdVDthH7HnAZRE9JYk=";
    };
  };
  kernelsu-stable-revision-code = {
    pname = "kernelsu-stable-revision-code";
    version = "11872";
    src = fetchurl {
      url = "https://example.com";
      sha256 = "sha256-6o+sfGX7WJsNU1YPUlH3T56bJDR43Laz6nm142RJyNk=";
    };
  };
  linux-amazon-karnak = {
    pname = "linux-amazon-karnak";
    version = "e22c713c7c4f0c8d08267f6bba98b8c4cde8310f";
    src = fetchFromGitHub {
      owner = "mt8163";
      repo = "android_kernel_amazon_karnak_4.9";
      rev = "e22c713c7c4f0c8d08267f6bba98b8c4cde8310f";
      fetchSubmodules = false;
      sha256 = "sha256-viqlQBPd2SuiAphFKiwjtckC3gDTGkbLv2dgZKMP3Oc=";
    };
    date = "2023-10-09";
  };
  linux-moto-pstar-lineageos-21 = {
    pname = "linux-moto-pstar-lineageos-21";
    version = "3c0c6944a81fff77848ac183870e595abd907a01";
    src = fetchFromGitHub {
      owner = "LineageOS";
      repo = "android_kernel_motorola_sm8250";
      rev = "3c0c6944a81fff77848ac183870e595abd907a01";
      fetchSubmodules = false;
      sha256 = "sha256-0CsNyIaFBxSw5rHCv89qHTkma2938N7lQRPv8GSE9A8=";
    };
    date = "2024-11-30";
  };
  linux-moto-rtwo-lineageos-21 = {
    pname = "linux-moto-rtwo-lineageos-21";
    version = "1c2aa53515b7c1d5629a84980470d9c1c2ece4e7";
    src = fetchFromGitHub {
      owner = "moto-sm8550";
      repo = "android_kernel_motorola_sm8550";
      rev = "1c2aa53515b7c1d5629a84980470d9c1c2ece4e7";
      fetchSubmodules = false;
      sha256 = "sha256-1czfscSPxDELAfyeNmrFG0x2GfCOl8CHe2C7GOsfR40=";
    };
    date = "2024-02-23";
  };
  linux-oneplus-8t-blu-spark = {
    pname = "linux-oneplus-8t-blu-spark";
    version = "2248fcf83d2a19ac0adb12bc6ed319b31090f6f4";
    src = fetchFromGitHub {
      owner = "engstk";
      repo = "op8";
      rev = "2248fcf83d2a19ac0adb12bc6ed319b31090f6f4";
      fetchSubmodules = false;
      sha256 = "sha256-ofzdtkWRcPiXWvzoonv654lyDM7NsWdBE8m1JxmnGMk=";
    };
    date = "2024-11-14";
  };
}
